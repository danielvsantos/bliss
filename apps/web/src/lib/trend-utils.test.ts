...
describe('computeTrendMovingAverages', () => {
  it('adds no __avg keys when there are fewer than 3 rows', () => {
    const rows = [
  ...
  it('adds the average only from the 3rd row onward, computed over the trailing window', () => {
    const rows = [
      { name: '2023-01', 'Dining Out': 100 },
      { name: '2023-02', 'Dining Out': 200 },
    ...
    const result = computeTrendMovingAverages(rows, ['Dining Out']);
    expect(result[0]).not.toHaveProperty('Dining Out__avg');
    expect(result[1]).not.toHaveProperty('Dining Out__avg');
    expect(result[2]['Dining Out__avg']).toBe(200); // mean(100, 200, 300)
  });

  it('computes a correct trailing window per row per group across 5+ rows and 2 groups', () => {
    const rows = [
    ...
    const result = computeTrendMovingAverages(rows, ['A', 'B']);
  ...
  it('includes a 0 value in the window instead of skipping it', () => {
    const rows = [
