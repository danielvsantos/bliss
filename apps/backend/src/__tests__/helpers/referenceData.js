 ...
 *
 * CI's `bliss_test` is created with `prisma db push` / `migrate deploy` only —
 * no `prisma db seed` runs — so these non-tenant-scoped tables start empty.
 * Any integration test that creates an `Account` (and therefore a
 * `Transaction`) must ensure the FK targets exist first.
 *
