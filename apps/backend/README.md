...
-   **AI Classification & Insights**: Provider-agnostic adapter layer at `src/services/llm/` supporting [Google Gemini](https://ai.google.dev/), [OpenAI](https://platform.openai.com/), and [Anthropic Claude](https://console.anthropic.com/). A factory resolves the configured provider at module load based on `LLM_PROVIDER` / `EMBEDDING_PROVIDER` env vars. See `docs/specs/backend/20-llm-provider-abstraction.md`.
...
| `EMBEDDING_PROVIDER` | Override embedding provider — **required when `LLM_PROVIDER=anthropic`** since Anthropic has no embedding API. |
...
-   **/src/workers**: BullMQ worker implementations that listen to the queues and process jobs. Core business logic lives here.
    ...
    -   `insightGeneratorWorker.js` — Tiered insight generation. Job types: `generate-all-insights` (cron `0 6 * * *` — daily heartbeat that auto-triggers MONTHLY / QUARTERLY / ANNUAL when their calendar window is open), `generate-portfolio-intel` (cron `0 5 * * 1` — weekly Monday equity intelligence via `SecurityMaster`), and `generate-tenant-insights` (on-demand, accepts `{ tier, year, month, quarter, periodKey, force }`). For each job, the worker calls `dataCompletenessService.checkCompleteness()`, gathers tier-specific data via `insightService`, calls the configured LLM provider's insight model via `services/llm/` (`INSIGHT_MODEL` override), dedupes by `(tenantId, tier, periodKey, dataHash)`, and persists additively with dismissed-state preservation.
...
-   **/src/routes**: Express route definitions for the internal API.
    ...
    -   `insights.js` — Internal insight endpoints. `POST /api/insights/generate` accepts `{ tenantId, tier, year, month, quarter, periodKey, force }`, validates the tier against `VALID_TIERS` (`MONTHLY | QUARTERLY | ANNUAL | PORTFOLIO`) and enforces per-tier required params (year+month for MONTHLY, year+quarter for QUARTERLY, year for ANNUAL), then enqueues a `generate-tenant-insights` job on the insights queue. `POST /api/insights/cleanup` triggers TTL-based removal of expired insights via `insightRetentionService.cleanupExpiredInsights()` and returns `{ deletedCount }`.
...
All 8 BullMQ workers report failed jobs to Sentry via `worker.on('failed')` with `Sentry.withScope()`. Each error is enriched with structured context: worker name, job name, job ID, tenant ID, and `attemptsMade`.
