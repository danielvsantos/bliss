...
**Minimum length: 32 characters, enforced at startup.** Each of the four required secrets is checked by `validateEnv()` in both the API and the backend. In production a short secret is a fatal error and the service refuses to boot; in development it is a warning. `scripts/setup.sh` generates 48/48/48/32, so a generated `.env` always passes. To create one by hand: `openssl rand -base64 48`.

The check is length only — deliberately no entropy heuristic, which produces false negatives on legitimately random strings and would be a boot-time landmine on a running instance.
...
| `JWT_SECRET_CURRENT` | Yes | -- | HMAC secret used to sign and verify JWT access tokens. Minimum 32 characters. |
...
When the frontend and API are served from different origins (e.g., `https://app.example.com` and `https://api.example.com`), Bliss automatically switches the NextAuth session cookies to `SameSite=None; Secure`. This is required because the OAuth sign-in flow involves a cross-origin form POST — browsers block `SameSite=Lax` cookies on those requests, which would fail the CSRF check.

The switch is gated on `NEXTAUTH_URL`: if it starts with `https://`, the cross-domain cookie configuration is applied; otherwise NextAuth defaults (`SameSite=Lax`) are used. This means:

- **HTTP deployments** (same-domain or cross-domain): `SameSite=Lax` is used. Google OAuth works if the API and frontend share the same origin; it will not work across HTTP origins (this is also a Google policy restriction).
- **HTTPS same-domain**: `SameSite=None; Secure` is used. No regression — same-origin cookies are always sent regardless of SameSite policy.
