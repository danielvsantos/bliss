...
The API test suite consists of **67 unit test files** and **14 integration test files** for a total of **621 tests across 81 files**. Unit tests cover middleware, utilities, and route handlers across `unit/middleware/`, `unit/utils/`, and `unit/api/` directories. Integration tests use both real-DB and mocked-handler patterns. Run `pnpm test:api` to execute all tests.
